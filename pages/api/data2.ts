...
// download allEvents of data.json content to the data table
    ...
    console.log(  JSON.stringify(event.emails_registered))
      await client.sql`
        INSERT INTO data6 (id, city, title, description, image, emails_registered)
        VALUES (${event.id}, ${event.city}, ${event.title}, ${event.description}, ${event.image}, ARRAY[${JSON.stringify(event.emails_registered)}]);`;
    })
